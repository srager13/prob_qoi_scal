function [ pdf_TF ] = pdf_TF_ftn_2( N, i, j, x )
...
if i < ceil(N/2) 
    if j < i
        mu_1 = ((i-1)*(N-i))/(N);
        sigma_1 = sqrt((((i-1)*(N-i))/(N))*(1-(1.0/(N))));
        pdf_TF = normpdf(x, mu_1, sigma_1);
    end

    if i < j && j < ceil(N/2)
        mu_2 = ((j-1)*(N-j))/(N);
        sigma_2 = sqrt((((j-1)*(N-j))/(N))*(1-(1.0/(N))));
        pdf_TF = normpdf(x, mu_2, sigma_2);
    end

    if j >= ceil(N/2)
        mu_3 = ((ceil(N/2)-1)*ceil(N/2))/(N);
        sigma_3 = sqrt((((ceil(N/2)-1)*ceil(N/2))/(N))*(1-(1.0/(N))));
