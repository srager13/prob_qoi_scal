...
% [fitresult, gof] = expected_TF_grid_qoi_feas();
% p00 = fitresult.p00;
% p01 = fitresult.p01;
% p02 = fitresult.p02;
% p03 = fitresult.p03;
...
for i=1:size(num_images,2)
    for j=1:length(T)
        ...
        sol_1 = solve( W*T(j) - C*(sqrt(N)*num_images(1,i))*B - (2.5*C*((2/3)*sqrt(N))*P_s) == 0, N );
        % node to all others
%         sol_1 = solve( W*T(j) - C*B*N*((3/4)*sqrt(N)-1) - ((1+2+3+4)*(1/4)*C*((2/3)*sqrt(N))*P_s) == 0, N );
...
if plot_3d == 0
    ...
    saveas(gcf, sprintf('./image_size_%i_KB/channel_rate_%i/grid_net/scal_feasible_reg_grid_flood.pdf', IS, channel_rate/1000000));
end
