...
directory = sprintf( './scal_predictions/image_size_%i_KB/channel_rate_%i/PS_%i/grid_net', image_size, channel_rate, packet_size );
...
if plot_color == 1
    saveas(gcf, sprintf('%s/grid_scal_anal_vs_sim_color_ss_%.1f%s.pdf', directory, sum_sim_values(ss_plot_index), avg_all_flows));
    savefig(sprintf('%s/grid_scal_anal_vs_sim_color_ss_%.1f%s.fig', directory, sum_sim_values(ss_plot_index), avg_all_flows));
end
